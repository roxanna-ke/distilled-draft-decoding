...
def reverse_kl(
    ...
    p = F.log_softmax(teacher_logits / temperature, dim=-1)  # teacher log
    q = F.softmax(student_logits / temperature, dim=-1)       # student prob
    q_log = F.log_softmax(student_logits / temperature, dim=-1)  # student log
    ...
    return (per_pos_rkl * mask).sum() / n_valid * (temperature ** 2)
