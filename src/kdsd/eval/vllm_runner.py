...
def _import_vllm() -> tuple[type, type]:
    try:
        ...
        raise RuntimeError(
            "eval.backend=vllm requires vLLM. Source scripts/env.sh first so "
            "KDSD_PYTHON points at the /scratch/venvs/kdsd-vllm environment."
        ) from exc
    return LLM, SamplingParams


def _vllm_sampling_params(sampling_params_cls: type, *, runtime: dict, seed: int | None) -> Any:
...
def _create_vllm_engine(
    *,
    ...
    kwargs: dict[str, Any] = {
        ...
        # Keep stat logging enabled so speculative counters can be read.
...
def _batched_generate(
    *,
    ...
    for i in range(0, len(prompts), request_batch_size):
        outputs.extend(llm.generate(prompts[i : i + request_batch_size], sampling_params, use_tqdm=False))
...
def _run_repeated_generation(
    *,
    ...
    return outputs, float(total_s), _count_output_tokens(outputs)
...
def _metric_value(metric: Any) -> float:
    try:
        return float(getattr(metric, "value", 0))
...
def run_vllm_eval(
    *,
    ...
    sampling_params = _vllm_sampling_params(sampling_params_cls, runtime=runtime, seed=seed)
    ...
    if do_vanilla:
        ...
        try:
            _, v_total_s, v_total_tokens = _run_repeated_generation(
                llm=vanilla_llm,
                prompts=formatted_prompts,
                sampling_params=sampling_params,
                request_batch_size=request_batch_size,
    ...
    for name in benchmarks:
        try:
            ...
            LOG.warning("benchmark %s not registered: %s - skipping", name, exc)
            continue
        try:
            quality_score[name] = float(cls().score(rows, None))
        except Exception as exc:
            LOG.warning("benchmark %s failed: %s - skipping", name, exc)
    ...
    engine_metrics = {
        ...
        "vanilla_tokens_per_second": float(vanilla_tps) if not math.isnan(vanilla_tps) else None,
