#!/bin/bash
# Run Qwen3-8B / Qwen3-0.6B RKL training, then evaluate the pretrained and
# trained draft models with vLLM speculative decoding.
...
run_draft_eval() {
  ...
  "${KDSD_PYTHON}" scripts/evaluate_sd.py \
    ...
    "eval.run_vanilla_baseline=false" \
...
if [[ ("${RUN_EVAL}" == "true" || "${RUN_EVAL}" == "1") && ("${EVAL_PRETRAINED_BASELINE}" == "true" || "${EVAL_PRETRAINED_BASELINE}" == "1") ]]; then
  ...
  run_draft_eval "${baseline_eval_run}" "${DRAFT_ID}" "${baseline_results_dir}"
...
if [[ "${RUN_EVAL}" == "true" || "${RUN_EVAL}" == "1" ]]; then
  ...
  run_draft_eval "${eval_run}" "${checkpoint_dir}/model" "${eval_results_dir}"
