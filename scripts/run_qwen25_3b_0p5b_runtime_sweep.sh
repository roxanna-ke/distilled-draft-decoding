#!/bin/bash
# Run a sequential vLLM runtime sweep for the Qwen2.5-3B/0.5B model pair
# across four KD draft checkpoints.
#
# The sweep covers gamma in {1,2,4} and max_new_tokens in {128,256}.
...
PRETRAINED_DRAFT_ID="${PRETRAINED_DRAFT_ID:-Qwen/Qwen2.5-0.5B-Instruct}"
...
EVAL_PROMPTS_LIMIT="${EVAL_PROMPTS_LIMIT:-200}"
KEEP_INTERMEDIATE_RESULTS="${KEEP_INTERMEDIATE_RESULTS:-false}"

GAMMAS="${GAMMAS:-1 2 4}"
MAX_NEW_TOKENS_VALUES="${MAX_NEW_TOKENS_VALUES:-128 256}"
RUN_NAME_PREFIX="${RUN_NAME_PREFIX:-qwen25_3b_0p5b_runtime_sweep}"
DRAFT_CHECKPOINTS="${DRAFT_CHECKPOINTS:-fkl_target_gen=${CHECKPOINTS_ROOT}/qwen25_3btarget_0p5b_fkl_ultrachat_50k_target_gen_seed42
jsd_target_gen=${CHECKPOINTS_ROOT}/qwen25_3btarget_0p5b_jsd_ultrachat_50k_target_gen_seed42
rkl_target_gen=${CHECKPOINTS_ROOT}/qwen25_3btarget_0p5b_rkl_ultrachat_50k_target_gen_seed42}"
...
while IFS= read -r checkpoint_entry; do
  [[ -z "${checkpoint_entry}" ]] && continue

  draft_label="${checkpoint_entry%%=*}"
  checkpoint_path="${checkpoint_entry#*=}"

  if [[ -z "${draft_label}" || -z "${checkpoint_path}" || "${draft_label}" == "${checkpoint_path}" ]]; then
    echo "ERROR: invalid DRAFT_CHECKPOINTS entry: ${checkpoint_entry}" >&2
    exit 1
  fi

  if ! draft_spec="$(resolve_draft_spec "${checkpoint_path}")"; then
    echo "ERROR: draft checkpoint not found: ${checkpoint_path}" >&2
    exit 1
  fi

  echo ">>> Resolved draft ${draft_label}: ${draft_spec}"
