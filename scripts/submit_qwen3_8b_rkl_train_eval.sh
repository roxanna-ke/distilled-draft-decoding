#!/bin/bash
# Submit one RunAI job that trains a Qwen3-0.6B RKL draft for a Qwen3-8B
# target, then evaluates the pretrained and trained drafts with vLLM.
...
RUN_NAME_PREFIX="${RUN_NAME_PREFIX:-qwen3_8btarget_0p6b_rkl}"
...
run_command+=" bash scripts/run_qwen3_8b_rkl_train_eval.sh"

echo ">>> RUN_COMMAND chars: ${#run_command}"
echo ">>> Submitting Qwen3 8B/0.6B RKL train+eval job: ${EXPERIMENT_NAME}"
REPO_URL="${REPO_URL}" \
...
./rcp_support/submit_train.sh "qwen3-8b-rkl-train-eval"
