#!/bin/bash
# Submit one RunAI job that runs the Qwen2.5-3B/0.5B runtime sweep
# sequentially over four KD draft checkpoints.

set -euo pipefail

WORK_ROOT="${WORK_ROOT:-/scratch/cs552-mnlp-kzy}"
REPO_DIR="${REPO_DIR:-${WORK_ROOT}/repos/distilled-draft-decoding-qwen25-eval}"
...
run_command+=" DRAFT_CHECKPOINTS=$(quote "${DRAFT_CHECKPOINTS}")"
run_command+=" bash scripts/run_qwen25_3b_0p5b_runtime_sweep.sh"

echo ">>> RUN_COMMAND chars: ${#run_command}"
echo ">>> Submitting Qwen2.5-3B/0.5B four-draft runtime sweep: ${EXPERIMENT_NAME}"
REPO_URL="${REPO_URL}" \
...
./rcp_support/submit_train.sh "qwen25-3b-runtime-sweep"
